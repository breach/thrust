...
std::string 
BreachDevToolsDelegate::GetDiscoveryPageHTML() 
{
  return ResourceBundle::GetSharedInstance().GetRawDataResource(
      IDR_BREACH_DEVTOOLS_DISCOVERY_PAGE).as_string();
}

bool 
BreachDevToolsDelegate::BundlesFrontendResources() 
{
  return true;
}

base::FilePath 
BreachDevToolsDelegate::GetDebugFrontendDir() 
{
  return base::FilePath();
}

std::string 
BreachDevToolsDelegate::GetPageThumbnailData(const GURL& url) 
{
  return std::string();
}
...
BreachDevToolsDelegate::GetTargetType(RenderViewHost*) {
  return kTargetTypeTab;
}

std::string BreachDevToolsDelegate::GetViewDescription(
    content::RenderViewHost*) {
  return std::string();
}

scoped_refptr<net::StreamListenSocket>
BreachDevToolsDelegate::CreateSocketForTethering(
