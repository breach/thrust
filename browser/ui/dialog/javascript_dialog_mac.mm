...
@interface JavaScriptDialogHelper : NSObject<NSAlertDelegate> {
 @private
  ...
  breach::BreachJavaScriptDialogManager* manager_;
  JavaScriptDialogManager::DialogClosedCallback callback_;
}

- (id)initHelperWithManager:(breach::BreachJavaScriptDialogManager*)manager
   andCallback:(JavaScriptDialogManager::DialogClosedCallback)callback;
- (NSAlert*)alert;
...
- (void)alertDidEnd:(NSAlert*)alert
  ...
  breach::JavaScriptDialog* native_dialog =
      reinterpret_cast<breach::JavaScriptDialog*>(contextInfo);
